 ...
 *     MA5_STREAK_EXIT 连续 ≥2 日 全天站上 5 日线（最低价也不触线）→ 提醒卖出
...
(function (root, factory) {
  ...
  /* ★ R3（2026-09-17）：MA5_STREAK_EXIT 增加「均线多头排列」前提（MA5>MA10>MA20>MA30）。
   ★ R4（2026-09-24）：「站上 5 日线」口径收紧为**全天都在** —— 最低价（影线）也不许
     小于等于 5 日线（用户用银禧科技案例校准：只比开收、影线触线的乖离不算站上）。
  ...
  function rMa5StreakExit(bars, i, ind) {
    ...
    return {
      ...
      title: '连续站上 5 日线',
      detail: '连续 ' + st.days + ' 天全天都站上 5 日线（影线也不触线，' + st.from + ' 起），且均线多头排列' +
  ...
  function ma5Streak(bars, endIdx) {
    ...
    for (var k = i; k >= 0; k--) {
      var m = maAt(bars, k, P.biasMa);
      if (!isFinite(m)) break;
      /* ★ 2026-09-24 用户校准（银禧科技案例）：站上 = **全天**都在 5 日线上 ——
         最低价（含上下影线）都不许小于等于 5 日线。最低价是当日四价的下界，
         low > MA5 就自动蕴含开盘、收盘都在上方；只比开收、影线触线的情况（乖离）不算站上。 */
      var lo = num(bars[k].l);
